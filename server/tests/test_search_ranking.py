...
async def test_search_defaults_to_alphabetical(auth_client):
    ...
    ids = [r["id"] for r in (await auth_client.get("/foods/search", params={"q": "Zqxunique"})).json()]
...
async def test_recently_logged_food_ranks_first(auth_client):
    ...
    ids = [r["id"] for r in (await auth_client.get("/foods/search", params={"q": "Qwtunique"})).json()]
...
async def test_ranking_is_per_user(auth_client, client):
    ...
    ids = [
        r["id"]
        for r in (await client.get("/foods/search", params={"q": "Rvtunique"}, headers=headers)).json()
    ]
